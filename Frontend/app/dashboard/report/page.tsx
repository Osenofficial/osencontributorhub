...
export default function ReportPage() {
  ...
  async function handleExportCsv() {
    ...
    try {
      ...
      if (from) params.set('from', from)
      if (to) params.set('to', to)
      const csv = await apiFetchText(`/dashboard/report/export-csv?${params.toString()}`)
      downloadCsv(`contribution-report-${new Date().toISOString().slice(0, 10)}.csv`, csv)
    } catch (err: any) {
        ...
        <form onSubmit={handleSearch} className="glass rounded-2xl border p-5 space-y-4">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:flex-wrap lg:items-end">
            <Input
              ...
              className="bg-input border-border/60 text-sm lg:min-w-[220px] lg:flex-1"
            />
            <Popover>
        ...
        {report && (
          <div className="space-y-4">
            ...
            <div className="glass rounded-2xl border overflow-hidden">
              <div className="px-5 py-3.5 border-b border-border/50 flex items-center justify-between gap-2 flex-wrap">
                <h3 className="text-sm font-semibold">Tasks in range</h3>
                <div className="flex items-center gap-2">
                  <Button type="button" variant="ghost" size="sm" className="gap-1.5 h-8" disabled={exportingCsv} onClick={handleExportCsv}>
                    <Download className="size-3.5" />
